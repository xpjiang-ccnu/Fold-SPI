...
parser = argparse.ArgumentParser(description='Fold-SPI train 5 fold')
parser.add_argument('--feature_dir', default='../data_prepare/STRING_data/Fold_SPI', type=str)
parser.add_argument('--data_file', default='../data_prepare/STRING_data/SPI/actions_all_1.txt', type=str)
parser.add_argument('--label_file', default='Fold_SPI_label_1.txt', type=str)
parser.add_argument('--preds_file', default='Fold_SPI_preds_1.txt', type=str)
...
for train_index, test_index in kf.split(dataset):
    ...
    for e in range(EPOCHS):
        ...
        if best_test_aupr < test_AUPR:
            ...
            np.savetxt('../result/STRING_data/' + parser.parse_args().label_file, labels, delimiter='\n', fmt='%s')
            np.savetxt('../result/STRING_data/' + parser.parse_args().preds_file, test_preds, delimiter='\n', fmt='%s')
    ...
    checkpoint = {'model': Fold_SPI(), 'model_state_dict': model.state_dict()}
    torch.save(checkpoint, f'../ckpts/STRING/model_cv_ckpts_{fold}.pkl')
